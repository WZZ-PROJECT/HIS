<template>
  ...
  <el-container style="padding: 0px;margin:0 0 0 0">
    <!-- 列表 -->
    <el-aside width="30%" style="padding:0 5px 0 0;background:white;border-style: dotted;border-width: 0px 1px 0px 0px;border-color:#C0C0C0;" >
      ...
      <pagination v-show="menuTotal>0" :total="menuTotal" :page.sync="menuListQuery.pageNum" :limit.sync="menuListQuery.pageSize" @pagination="getMenu" />
    </el-aside>
    <!-- 每一列的表格详情 -->
    <el-main style="margin-left:10px">
      <el-button disabled style="color:black">当前类别：{{currentRow.name}}</el-button>
      <el-button type="text" v-if="currentRow.name!=='请点击左侧选择诊断类别'" @click="dialogVisibleDis=true;dialogType='add';onedis.catId=currentRow.id"><i class="el-icon-circle-plus" />新增诊断</el-button>
