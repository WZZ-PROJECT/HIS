  ...
  <div>
    ...
    <el-container>
      ...
      <el-main v-if="modeldetail">
        <el-button type="primary" @click="changeModel('editmodel')">保存修改</el-button>
        <el-form :model="editmodel" ref="editmodel" style="width:900px;margin-top:30px" inline label-width="100px" :rules="rule">
          <el-form-item  label="模板名" prop="name">
            <el-input placeholder="模板名" v-model="editmodel.name" style="width:400px"></el-input>
          </el-form-item>
          <el-form-item label="主述" prop="chiefComplaint">
          ...
          <el-form-item label="现病治疗情况" prop="historyOfTreatment">
    ...
    <el-dialog title="新增模板/目录" :visible.sync="addnodevisible" width="300px">
      <el-form :model="newmodel" :rules="rules" ref="ruleForm">
        ...
        <el-form-item>
          ...
          <el-button type="primary" @click="append('ruleForm')" style="float:right">确定</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    ...
    data() {
      return {
        ...
        rule: {
          ...
          chiefComplaint: [{required: true, message: '请输入主述', trigger: 'blur'},
            {min: 4, max: 16, message: '长度在 4 到 16 个字符', trigger: 'blur'}],
          historyOfPresentIllness: [{required: true, message: '请输入现病史', trigger: 'blur'},
            {min: 4, max: 16, message: '长度在 4 到 16 个字符', trigger: 'blur'}],
          historyOfTreatment: [{required: true, message: '请输入现病治疗情况', trigger: 'blur'},
            {min: 4, max: 16, message: '长度在 4 到 16 个字符', trigger: 'blur'}],
          pastHistory: [{required: true, message: '请输入既往史', trigger: 'blur'},
            {min: 4, max: 16, message: '长度在 4 到 16 个字符', trigger: 'blur'}],
          allergies: [{required: true, message: '请输入过敏史', trigger: 'blur'},
            {min: 4, max: 16, message: '长度在 4 到 16 个字符', trigger: 'blur'}],
          healthCheckup: [{required: true, message: '请输入体格检查', trigger: 'blur'},
            {min: 4, max: 16, message: '长度在 4 到 16 个字符', trigger: 'blur'}]
        },
        rules: {
