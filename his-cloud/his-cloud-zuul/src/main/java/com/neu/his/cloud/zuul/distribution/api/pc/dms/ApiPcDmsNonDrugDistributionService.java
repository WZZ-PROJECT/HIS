...
public interface ApiPcDmsNonDrugDistributionService {
    ...
    CommonResult<CommonPage<DmsNonDrugResult>> listAll(@RequestParam(value = "pageSize", defaultValue = "5") Integer pageSize,
                                                       @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum);

    @ApiOperation(value = "查询所有非药品")
    @RequestMapping(value = "/DmsNonDrug/listDmsNonDrugAll", method = RequestMethod.POST)
    @ResponseBody
    CommonResult<List<DmsNonDrugResult>> listDmsNonDrugAll();
}
