...
public class DmsNonDrugDistributionController {
    ...
    public CommonResult<CommonPage<DmsNonDrugResult>> listAll( @RequestParam(value = "pageSize", defaultValue = "5") Integer pageSize,
                                                         @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum){
        return apiPcDmsNonDrugDistributionService.listAll(pageSize,pageNum);
    }
    private CommonResult<CommonPage<DmsNonDrugResult>> listAllFallbackInfo(Integer pageSize, Integer pageNum){
        return CommonResult.success(null,"请检查您的网络") ;
    }

    @HystrixCommand(fallbackMethod = "listDmsNonDrugAllFallbackInfo")
    @ApiOperation(value = "查询所有非药品")
    ...
    public CommonResult<List<DmsNonDrugResult>> listDmsNonDrugAll(){
        return apiPcDmsNonDrugDistributionService.listDmsNonDrugAll();
    }

    public CommonResult<List<DmsNonDrugResult>> listDmsNonDrugAllFallbackInfo(){
        return CommonResult.success(null,"请检查您的网络") ;
    }
}
