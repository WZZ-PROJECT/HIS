...
public class DmsNonDrugItemRecordDistributionController {
    ...
    @HystrixCommand(fallbackMethod = "deleteByIdFallbackInfo")
    @ApiOperation(value = "作废项目")
    @RequestMapping(value = "/deleteById", method = RequestMethod.POST)
    @ResponseBody
    public CommonResult deleteById(@RequestParam("ids") List<Long> ids){
        return apiPcDmsNonDrugItemRecordDistributionService.deleteById(ids);
    }
    private CommonResult deleteByIdFallbackInfo(List<Long> ids){
        return CommonResult.success(null,"请检查您的网络") ;
    }
}
