...
public class SmsSkdServiceImpl implements SmsSkdService {
    ...
    public List<SmsSkdResult> listSkd(SmsSkdParam smsSkdParam){
        ...
        if (smsSkdParam.getStartDate() != null && smsSkdParam.getStartDate().compareTo(parse)!=0) {
      ...
      /* if(smsSkdParam.getStaffName()!=null){
            SmsStaffExample smsStaffExample=new SmsStaffExample();
            smsStaffExample.createCriteria().andNameEqualTo(smsSkdParam.getStaffName());
            List<SmsStaff> smsStaffs = smsStaffMapper.selectByExample(smsStaffExample);
            if(!CollectionUtils.isEmpty(smsStaffs)){
                criteria.andStaffIdEqualTo(smsStaffs.get(0).getId());
            }
        }*/
        ...
        if(smsSkdParam.getRegistrationRankId() != null){
            ...
            if (smsSkdParam.getStaffName() != null) {
                staffCriteria.andNameLike("%"+smsSkdParam.getStaffName()+"%");
            }
            if (StringUtils.isEmpty(smsSkdParam.getRegistrationRankId())) {
