...
//    }
     ...
     */
    @Override
    public int bindPatient(PmsPatient pmsPatient) {
        PmsPatientExample pmsPatientExample=new PmsPatientExample();
        pmsPatientExample.createCriteria().andIdentificationNoEqualTo(pmsPatient.getIdentificationNo());
        PmsPatient pmsPatients = pmsPatientMapper.selectByExample(pmsPatientExample).get(0);
        pmsPatients.setOpenId(pmsPatient.getOpenId());
        pmsPatients.setPicture(pmsPatient.getPicture());
        return pmsPatientMapper.updateByPrimaryKey(pmsPatients);
    }
}
