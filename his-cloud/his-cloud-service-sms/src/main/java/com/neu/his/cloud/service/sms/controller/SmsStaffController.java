...
import com.neu.his.cloud.service.sms.model.PmsPatient;
...
//    }
    ...
    //将患者和微信号绑定（更新Patient表的openID字段）
    @ApiOperation("将患者和微信号绑定（更新Patient表的openID字段）")
    ...
    public CommonResult bindPatient(@RequestBody PmsPatient pmsPatient){
        int count = smsStaffService.bindPatient(pmsPatient);
        if(count > 0){
