...
public class BmsFeeServiceImpl implements BmsFeeService {
    ...
    public List<BmsRegistrationPatientResult> listRegisteredPatient(String medicalRecordNo, Date queryDate) {
        ...
        if (medicalRecordNo == null && queryDate == null){//如果病历号为空和挂号日期为空，则显示今天的挂号记录按创建时间倒序
            ...
            dmsRegistrationExample.createCriteria().andAttendanceDateBetween(getStartOfDay(startOfDate),getEndOfDay(startOfDate));
    ...
    public int charge(List<BmsChargeParam> bmsChargeParamList)  {
        ...
        if (bmsChargeParamList.isEmpty()){
            ...
            // 查询挂号人id
            DmsRegistration dmsRegistration = dmsRegistrationMapper.selectByPrimaryKey(registrationIds.get(0));
            if (!StringUtils.isEmpty(dmsRegistration)) {
                BmsAccountExample example = new BmsAccountExample();
                example.createCriteria().andPatientIdEqualTo(dmsRegistration.getPatientId());
                List<BmsAccount> bmsAccounts = accountMapper.selectByExample(example);

                if(!CollectionUtils.isEmpty(bmsAccounts)){

                    BigDecimal blance = bmsAccounts.get(0).getBlance();
                    BmsAccount account = bmsAccounts.get(0);
                    if (blance.compareTo(totalAmount) >= 0) {

                        account.setBlance(blance.subtract(totalAmount));
...
////                DmsRegistration dmsRegistration = dmsRegistrationMapper.selectByPrimaryKey(registrationIds.get(0));
////                if(bmsChargeParamList.get(0).getSettlementCatId()==6 && !StringUtils.isEmpty(dmsRegistration)){
////                    int i1 = WxPay(bmsChargeParamList.get(0),dmsRegistration);
////                    if(i1>0){
////                        return 1;
////                    }else {
////                        int i=1/i1;
...
////
    ...
    @Override
    public int WxPay(BmsChargeParam bmsChargeParam, DmsRegistration dmsRegistration) {
        //微信付款码支付
        try {
            if(!StringUtils.isEmpty(bmsChargeParam.getXbarCode())){
                BigDecimal multiply = bmsChargeParam.getAmount().multiply(new BigDecimal("100"));
                System.out.println(multiply.toString());
                Map<String, String> stringStringMap = MyWXPay.scanCodeToPay(bmsChargeParam.getXbarCode(),multiply.toString());
                ...
                if(!CollectionUtils.isEmpty(stringStringMap)){
                    ...
                    return wxResultsMapper.insert(wxResults);
    ...
    @Override
    public int WxPay(WXDmsRegistrationParam dmsRegistrationParam) {
        //微信付款码支付
        try {
            if(!StringUtils.isEmpty(dmsRegistrationParam.getBarCode())){
                BigDecimal multiply = dmsRegistrationParam.getAmount().multiply(new BigDecimal("100"));
                System.out.println(multiply.toString());
                Map<String, String> stringStringMap = MyWXPay.scanCodeToPay(dmsRegistrationParam.getBarCode(), multiply.toString());
     ...
     */
    @Override
    public List<BmsResult> listCharge(BmsParam bmsParam) {

        SimpleDateFormat simpleDateFormat=new SimpleDateFormat("yyyy-MM-dd HH:mm");
        List<BmsResult> bmsResultList = new ArrayList<>();
        DmsRegistrationExample dmsRegistrationExample = new DmsRegistrationExample();
        dmsRegistrationExample.createCriteria().andPatientIdEqualTo(bmsParam.getPatientId());
        List<DmsRegistration> dmsRegistrations = dmsRegistrationMapper.selectByExample(dmsRegistrationExample);
        PmsPatientExample example = new PmsPatientExample();
        example.createCriteria().andIdEqualTo(bmsParam.getPatientId());
        List<PmsPatient> pmsPatients = pmsPatientMapper.selectByExample(example);
        if (dmsRegistrations.size() <= 0 ) {
            return bmsResultList;
        }
        dmsRegistrations.forEach(dmsRegistration -> {

            //非药品
            DmsNonDrugItemRecordExample dmsNonDrugItemRecordExample = new DmsNonDrugItemRecordExample();
            dmsNonDrugItemRecordExample.createCriteria().andRegistrationIdEqualTo(dmsRegistration.getId());
            dmsNonDrugItemRecordExample.setOrderByClause("create_time asc");
            List<DmsNonDrugItemRecord> dmsNonDrugItemRecordList = dmsNonDrugItemRecordMapper.selectByExample(dmsNonDrugItemRecordExample);
            if (!dmsNonDrugItemRecordList.isEmpty()){
                for (DmsNonDrugItemRecord dmsNonDrugItemRecord: dmsNonDrugItemRecordList) {
                    BmsResult bmsChargeResult = new BmsResult();
                    bmsChargeResult.setAttendanceDate(simpleDateFormat.format(dmsRegistration.getAttendanceDate()));
                    bmsChargeResult.setMedicalRecordNo(pmsPatients.get(0).getMedicalRecordNo());
                    bmsChargeResult.setId(dmsNonDrugItemRecord.getId());//是药品id或非药品id还是成药记录项id、草药记录项id、非药品记录项id（后者）
                    bmsChargeResult.setAmount(dmsNonDrugItemRecord.getAmount());
                    ...
                    bmsResultList.add(bmsChargeResult);
                }
            }
            //成药处方
            DmsMedicinePrescriptionRecordExample dmsMedicinePrescriptionRecordExample = new DmsMedicinePrescriptionRecordExample();
            dmsMedicinePrescriptionRecordExample.createCriteria().andRegistrationIdEqualTo(dmsRegistration.getId());
            dmsMedicinePrescriptionRecordExample.setOrderByClause("create_time asc");
            List<DmsMedicinePrescriptionRecord> dmsMedicinePrescriptionRecordList = dmsMedicinePrescriptionRecordMapper.selectByExample(dmsMedicinePrescriptionRecordExample);
            if (!dmsMedicinePrescriptionRecordList.isEmpty()){
                for(DmsMedicinePrescriptionRecord dmsMedicinePrescriptionRecord : dmsMedicinePrescriptionRecordList){
                    BmsResult bmsResult = new BmsResult();
                    bmsResult.setAttendanceDate(simpleDateFormat.format(dmsRegistration.getAttendanceDate()));
                    bmsResult.setMedicalRecordNo(pmsPatients.get(0).getMedicalRecordNo());
                    bmsResult.setId(dmsMedicinePrescriptionRecord.getId());
            ...
            //草药处方
            DmsHerbalPrescriptionRecordExample dmsHerbalPrescriptionRecordExample = new DmsHerbalPrescriptionRecordExample();
            dmsHerbalPrescriptionRecordExample.createCriteria().andRegistrationIdEqualTo(dmsRegistration.getId());
            dmsHerbalPrescriptionRecordExample.setOrderByClause("create_time asc");
            List<DmsHerbalPrescriptionRecord> dmsHerbalPrescriptionRecordList = dmsHerbalPrescriptionRecordMapper.selectByExample(dmsHerbalPrescriptionRecordExample);
            if (!dmsHerbalPrescriptionRecordList.isEmpty()){
                for (DmsHerbalPrescriptionRecord dmsHerbalPrescriptionRecord : dmsHerbalPrescriptionRecordList){//遍历处方
                    ...
                    bmsResult.setAttendanceDate(simpleDateFormat.format(dmsRegistration.getAttendanceDate()));
                    bmsResult.setMedicalRecordNo(pmsPatients.get(0).getMedicalRecordNo());
                    bmsResult.setId(dmsHerbalPrescriptionRecord.getId());
                    bmsResult.setCreateTime(simpleDateFormat.format(dmsHerbalPrescriptionRecord.getCreateTime()));
